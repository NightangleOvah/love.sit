(() => {
  ...
  style.textContent = `
    ...
    .console-inner{max-width:1100px;margin:auto;position:relative;z-index:3}.console-grid{display:grid;grid-template-columns:1.15fr .85fr;gap:18px;margin-top:45px}
    ...
    .ambient-pulse{position:fixed;z-index:-2;width:30vw;height:30vw;border-radius:50%;background:#a88cff12;filter:blur(55px);pointer-events:none;transition:transform 1s ease}
    .parallax-card{transform-style:preserve-3d;will-change:transform}.parallax-card>*{transform:translateZ(12px)}
    ...
    @media(max-width:800px){.console-grid{grid-template-columns:1fr}.planet-field{height:360px}.console-card{padding:18px}}
    @media(prefers-reduced-motion:reduce){.orbit-ring{animation:none}.shooting-star{display:none}}
  `;
  document.head.append(style);

  const section = document.createElement('section');
  section.className = 'universe-console chapter';
  section.id = 'deep-space';
  section.innerHTML = `
    <div class="ambient-pulse" id="ambientPulse"></div>
    <div class="console-inner">
      <p class="eyebrow">08 / DEEP SPACE</p>
      <h2 class="copy h2-like">A universe that<br><em>remembers.</em></h2>
      <p class="copy" style="color:var(--muted);max-width:650px">Explore the systems hidden underneath the scrapbook. Every discovery becomes part of this browser-local universe.</p>
      <div class="console-grid">
        <div class="console-card"><h3>Memory planets</h3><p>Four tiny worlds represent different kinds of memories. Tap one to reveal a new message.</p>
          <div class="planet-field" id="planetField"><span class="orbit-ring"></span><span class="orbit-ring"></span><button class="planet p1" data-planet="Adventure is a memory you can keep moving through."></button><button class="planet p2" data-planet="Quiet moments deserve stars too."></button><button class="planet p3" data-planet="The funniest memories are often the smallest ones."></button><button class="planet p4" data-planet="The future is another world waiting to be visited."></button></div>
          <div class="discovery" id="discovery">Choose a planet.</div>
        </div>
        <div class="console-card parallax-card"><h3>Universe controls</h3><p>Change the atmosphere, trigger a rare event, or reveal how many discoveries you have made.</p>
          ...
          <div class="discovery" id="achievement">0 / 4 memory worlds discovered</div>
          <div class="console-actions"><button id="keyboardHint">Keyboard guide</button></div>
        </div>
      </div>
    </div>`;
  document.querySelector('main')?.append(section);

  const discovery = section.querySelector('#discovery');
  const achievement = section.querySelector('#achievement');
  ...
  section.querySelectorAll('.planet').forEach((planet, i) => planet.addEventListener('click', () => { state.discovered.add(String(i)); discovery.textContent = planet.dataset.planet; save(); }));
  save();

  function meteor() {
    if (state.reduced) return;
    const el = document.createElement('i'); el.className = 'shooting-star'; el.style.left = `${20 + Math.random()*70}%`; el.style.top = `${5 + Math.random()*45}%`; document.body.append(el); setTimeout(() => el.remove(), 1400);
  }
  ...
  let nextMeteor = Date.now() + 7000;
  function ambient(time) { if (!state.reduced && !document.hidden && time > nextMeteor) { meteor(); nextMeteor = time + 9000 + Math.random()*16000; } requestAnimationFrame(ambient); }

(() => {
  ...
  const state = { discovered: Number(localStorage.getItem('olu-discovered-v2') || 0), easter: Number(localStorage.getItem('olu-easter-v2') || 0) };

  // --- Lightweight real WebGL heart/orbit scene ---------------------------------
  // It is deliberately self-contained: no external 3D library is required.
  ...
  let raf = 0;
  if (gl && !reduce) {
    const vs = `attribute vec3 a; uniform mat4 p; uniform float r; uniform vec2 m; void main(){float c=cos(r),s=sin(r); vec3 q=vec3(a.x*c-a.z*s,a.y,a.x*s+a.z*c); q.xy+=m*.05; q.z-=3.1; gl_Position=p*vec4(q,1.0); gl_PointSize=2.0+max(0.0,2.0-q.z*.18);}`;
  ...
  // --- Unusual navigation + chapter tracking -----------------------------------
  ...
  sections.forEach(s=>s.querySelectorAll(':scope > .copy, :scope > .archive-head, :scope > .timeline-head, :scope > .memory, :scope > .letter-stage, :scope > .value-orbit, :scope > .sky, :scope > .dreams').forEach((el,j)=>{el.classList.add('love-reveal');el.style.transitionDelay=Math.min(j*.06,.42)+'s'}));
  ...
  // Shooting stars: ambient and interactive.
  ...
  // --- Scroll depth --------------------------------------------------------------
  let last=0;const parallax=()=>{const y=scrollY;if(Math.abs(y-last)>2){document.querySelectorAll('.moon,.future-sun,.constellation-art,.voxel-sky').forEach(el=>{const r=el.getBoundingClientRect();if(r.bottom>0&&r.top<innerHeight)el.style.setProperty('--love-y',((r.top-innerHeight/2)*-.035)+'px')});last=y}requestAnimationFrame(parallax)};if(!reduce)requestAnimationFrame(parallax);
